...
def build_pc_shaders(config, n, platform)
  if platform.underscore_dirs_whitelist.include?("_vulkan")
    game_shaders = build_shader_pak_for_dir(config, n, config.effects_dir(false), config.pc_shader_dir(false), 'vulkan', '-iUsagi/Data/GLSL/Shaders')
    engine_shaders = build_shader_pak_for_dir(config, n, config.effects_dir, config.pc_shader_dir, 'vulkan', '')
  else
    game_shaders = build_shader_pak_for_dir(config, n, config.effects_dir(false), config.pc_shader_dir(false), 'ogl', '-iUsagi/Data/GLSL/Shaders')
